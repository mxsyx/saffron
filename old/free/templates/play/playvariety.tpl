...
<div class="container" id="container-playurl">
  <h3>播放线路
    <a href="/play/variety/{{ info.id }}/{{ next_episode }}" class="pn pull-right">【下一集】</a>
    <a href="/play/variety/{{ info.id }}/{{ previous_episode }}" class="pn pull-right">【上一集】</a>
  </h3>
  <ul class="itemlist">
    {% for num in nums %}
    {% if forloop.counter == episode %}
    <li id="itemlist-li-active">
      <a href="/play/variety/{{ info.id }}/{{forloop.counter}}">第{{forloop.counter}}集</a>
    </li>
    {% else %}
    <li>
      <a href="/play/variety/{{ info.id }}/{{forloop.counter}}">第{{forloop.counter}}集</a>
